...
def FindXGBMdls(X_train, X_test, y_train, y_test):
  param_grid = {
    'n_estimators': [50, 100, 150, 200, 250, 500],
    'learning_rate': [0.01, 0.05, 0.1, 0.2],
    'max_depth': [3, 4, 5, 6, 7],
...
_Describe your model and report the accuracy._
...
model = GradientBoostingClassifier(verbose=1, n_estimators=50)
...
```{python}
FindXGBMdls(X_trn, X_tst, y_trn, y_tst)
```

```{python}
# Gird Searching the Model to tune hyperparameters
FindMdls(X_trn, y_trn)
```
...
model_alt = GradientBoostingClassifier()
